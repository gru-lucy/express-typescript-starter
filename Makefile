...
start/docker/db: ## Start database container
	...
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env up -d express-typescript-starter-postgres express-typescript-starter-pgweb

.PHONY: stop/docker/db
stop/docker/db: ## Stop database container
	@echo "🛑 Stopping database (Docker)..."
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env stop express-typescript-starter-postgres express-typescript-starter-pgweb

.PHONY: start/cache
start/docker/cache: ## Start cache container
	@echo "▶️ Starting cache (Docker)..."
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env up -d express-typescript-starter-redis express-typescript-starter-redis-commander

.PHONY: stop/cache
stop/docker/cache: ## Stop cache container
	@echo "🛑 Stopping cache (Docker)..."
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env stop express-typescript-starter-redis express-typescript-starter-redis-commander
